...
- **Defense** — Into the Breach holdout. You defend a **city of 3–6 nexuses (2 HP each)**, packed in one or two clusters. There are no enemy crystals. Weak black-disc enemies telegraph, then after your turn they **execute**, **move**, and **telegraph** again, one body at a time. Each strike finishes — damage, death, and voids — before the next pawn winds up. They hunt the city first. New bodies stream in at a varying rate (quiet turns and busy turns, cap 7). **1–2** pawns can already be on the board at the start, with **no telegraph and no incoming marks** on round 1. You still bring four wizards from Pyre, Rime, and Squall. Drop a wizard **anywhere** on an open tile — they land **immediately**, burst (push only), then are spent that turn. The map is not mirrored. You lose if the cluster falls or your wizards are wiped. Clearing a wave does not end the fight.
