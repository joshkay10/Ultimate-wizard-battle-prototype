...
- **Stream / gust / bolt** — straight line, cardinals only. Stops on the first wizard or nexus. Mountains block the line. Water does not.
...
- **Raise** — empty walkable tile, not a portal. The mountain lasts about one round (through the opponent’s turn), then crumbles. Raised mountains are real walls: they block walk, summon, melee, cast, and push. They are not mirrored to the other camp.
- **Bolt silence** — the victim skips their next attack. No push.
- **Swap / blink** — target a wizard to swap places (line of sight not required; you can hop a mountain). Target an empty unblocked tile to blink there. Portals are legal blink landings. You cannot blink onto mountains, water, or nexuses.
...
- **Crash:** if the push hits a wall (edge, mountain, water, or nexus), the pushed wizard takes **1** damage. Leftover push distance does not add more.
