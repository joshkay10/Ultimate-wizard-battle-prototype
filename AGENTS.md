...
Named missions keep the remaining spawn budget after a wipe (killing the opener does not win). Clearing an island unlocks the next (`applyMissionClear`). Random Defense (no `missionId`) still ends on a wipe. Vs is free-play with the Team-page loadout. Mission fours are locked.
